...
std::optional<std::vector<std::byte>> getImageBytes(const fastgltf::Asset &asset, const fastgltf::Image &image,
                                                    const fs::path &basePath,
                                                    fastgltf::DefaultBufferDataAdapter &adapter) {
    return std::visit(
        fastgltf::visitor{[&](const fastgltf::sources::Array &array) -> std::optional<std::vector<std::byte>> {
...
GLuint uploadImageTexture(const fastgltf::Asset &asset, std::size_t imageIndex, const fs::path &basePath,
                          TextureCache &cache, bool srgb, fastgltf::DefaultBufferDataAdapter &adapter) {
...
GLuint resolveTexture(const fastgltf::Asset &asset, const fastgltf::TextureInfo &texInfo, const fs::path &basePath,
                      TextureCache &cache, bool srgb, fastgltf::DefaultBufferDataAdapter &adapter, GLuint fallback) {
...
glm::mat4 toMat4(const fastgltf::TRS &trs) {
    glm::mat4 T =
        glm::translate(glm::mat4(1.0f), glm::vec3(trs.translation[0], trs.translation[1], trs.translation[2]));
...
PrimitiveGPU uploadPrimitive(const fastgltf::Asset &asset, const fastgltf::Primitive &prim, const fs::path &basePath,
                             TextureCache &cache, fastgltf::DefaultBufferDataAdapter &adapter) {
    ...
    if (auto cutIt = prim.findAttribute("_CUT"); cutIt != prim.attributes.end()) {
        ...
        if (cutAcc.type != fastgltf::AccessorType::Scalar) {
            log::warning(
                log::format("Warning: _CUT accessor is not a Scalar (Type=", static_cast<int>(cutAcc.type), ")"));
        } else {
            std::size_t idx = 0;
            ...
            if (cutAcc.count != positions.size()) {
                log::warning(log::format("Warning: _CUT count (", cutAcc.count, ") != positions count (",
                                         positions.size(), ")"));
            }
        }
    }
    ...
    case fastgltf::PrimitiveType::Points:
        gpuPrim.mode = GL_POINTS;
        break;
    case fastgltf::PrimitiveType::Lines:
        gpuPrim.mode = GL_LINES;
        break;
    case fastgltf::PrimitiveType::LineLoop:
        gpuPrim.mode = GL_LINE_LOOP;
        break;
    case fastgltf::PrimitiveType::LineStrip:
        gpuPrim.mode = GL_LINE_STRIP;
        break;
    case fastgltf::PrimitiveType::TriangleStrip:
        gpuPrim.mode = GL_TRIANGLE_STRIP;
        break;
    case fastgltf::PrimitiveType::TriangleFan:
        gpuPrim.mode = GL_TRIANGLE_FAN;
        break;
    case fastgltf::PrimitiveType::Triangles:
    default:
        gpuPrim.mode = GL_TRIANGLES;
        break;
    }
    ...
    if (matPtr) {
        ...
        mgpu.baseColorTex = mat.pbrData.baseColorTexture ? resolveTexture(asset, *mat.pbrData.baseColorTexture,
                                                                          basePath, cache, true, adapter, cache.white)
                                                         : cache.white;
        mgpu.mrTex = mat.pbrData.metallicRoughnessTexture
                         ? resolveTexture(asset, *mat.pbrData.metallicRoughnessTexture, basePath, cache, false, adapter,
                                          cache.mrrDefault)
                         : cache.mrrDefault;
        mgpu.normalTex = mat.normalTexture ? resolveTexture(asset, *mat.normalTexture, basePath, cache, false, adapter,
                                                            cache.normalDefault)
                                           : cache.normalDefault;
        mgpu.emissiveTex = mat.emissiveTexture ? resolveTexture(asset, *mat.emissiveTexture, basePath, cache, true,
                                                                adapter, cache.emissiveDefault)
                                               : cache.emissiveDefault;

        log::info(
            log::format("Material: baseColorFactor=[", mgpu.baseColorFactor.r, ",", mgpu.baseColorFactor.g, ",",
                        mgpu.baseColorFactor.b, ",", mgpu.baseColorFactor.a, "] metallic=", mgpu.metallicFactor,
                        " roughness=", mgpu.roughnessFactor, " baseTex=", (mat.pbrData.baseColorTexture ? "yes" : "no"),
                        " mrTex=", (mat.pbrData.metallicRoughnessTexture ? "yes" : "no"),
                        " normalTex=", (mat.normalTexture ? "yes" : "no"),
                        " emissiveTex=", (mat.emissiveTexture ? "yes" : "no"), " unlit=", (mgpu.unlit ? "yes" : "no")));
        if (!mat.pbrData.metallicRoughnessTexture) {
            // No MR texture: honor the material's scalar roughness/metallic factors directly.
            log::info(log::format("Note: No MetallicRoughness-Texture, using roughness=", mgpu.roughnessFactor,
                                  " metallic=", mgpu.metallicFactor));
        }
    } else {
...
void expandBounds(glm::vec3 &minOut, glm::vec3 &maxOut, const glm::mat4 &m, const glm::vec3 &localMin,
...
void gatherDrawCommands(const fastgltf::Asset &asset, std::size_t nodeIndex, const glm::mat4 &parent,
...
std::optional<LoadedGltf> loadGltfAsset(const fs::path &gltfPath) {
    ...
    if (!dataResult) {
        log::error(
            log::format("Failed to load glTF: ", gltfPath, " (Error ", static_cast<int>(dataResult.error()), ")"));
        return std::nullopt;
    }
...
SceneGPU buildSceneGPU(const fastgltf::Asset &asset, const fs::path &basePath, TextureCache &texCache,
    ...
    if (whiteKeyMeshIndex)
        skipMeshes.insert(*whiteKeyMeshIndex);
    if (blackKeyMeshIndex)
        skipMeshes.insert(*blackKeyMeshIndex);

    std::function<void(std::size_t, const glm::mat4 &)> gatherFiltered = [&](std::size_t nodeIndex,
        ...
        if (node.cameraIndex) {
            ...
            if (!scene.camera.valid || isPreferred) {
                ...
                if (std::holds_alternative<fastgltf::Camera::Perspective>(cam.camera)) {
                    ...
                    if (p.aspectRatio.has_value())
                        scene.camera.aspect = *p.aspectRatio;
                    ...
                    log::info(log::format(
                        "Camera '", cam.name, "' pos=", eye.x, ",", eye.y, ",", eye.z, " forward=", forward.x, ",",
                        forward.y, ",", forward.z, " up=", up.x, ",", up.y, ",", up.z,
        ...
        if (node.meshIndex) {
            if (skipMeshes.count(*node.meshIndex)) {
                ...
                if (baseMeshIndex && *node.meshIndex == *baseMeshIndex && !baseModel)
                    baseModel = model;
...
void renderFrame(SceneGPU &scene, const GLProgram &pbrProgram, const SkyboxProgram &skyboxProgram,
    ...
    if (useKeyboardCamera) {
        ...
        float aspectCam = scene.camera.aspect.value_or(
    ...
    LightViewData lightView =
        computeLightView(scene, -lightDir, shadowMap.size); // light looks toward target along -lightDir
    ...
    auto drawDC = [&](const DrawCommand &dc, const glm::mat4 &modelOverride, bool enableStencilWrite,
                      bool disableCull) {
        auto instIt = scene.instancedMeshes.find(dc.meshIndex);
        const InstanceInfo *instInfo = (instIt != scene.instancedMeshes.end()) ? &instIt->second : nullptr;
...
bool Renderer::init(const InitParams &params) {
    ...
    if (std::filesystem::exists(params.envHdrPath)) {
        impl->texCache.envMap =
            loadEquirectangularAsCubemap(params.envHdrPath.string(), 2048, impl->skyboxVAO, impl->captureProg);
    }
...
void Renderer::render(const FrameParams &frame) {
    if (!impl || !impl->initialized)
        return;
    renderFrame(impl->scene, impl->pbrProg, impl->skyboxProg, impl->shadowProg, impl->shadowMap, impl->skyboxVAO, frame,
...
void Renderer::shutdown() {
    ...
    if (impl->shadowMap.depthTex)
        glDeleteTextures(1, &impl->shadowMap.depthTex);
    if (impl->shadowMap.fbo)
        glDeleteFramebuffers(1, &impl->shadowMap.fbo);
    if (impl->skyboxVAO)
        glDeleteVertexArrays(1, &impl->skyboxVAO);
    if (impl->pbrProg.id)
        glDeleteProgram(impl->pbrProg.id);
    if (impl->skyboxProg.id)
        glDeleteProgram(impl->skyboxProg.id);
    if (impl->shadowProg.id)
        glDeleteProgram(impl->shadowProg.id);
    if (impl->captureProg.id)
        glDeleteProgram(impl->captureProg.id);
