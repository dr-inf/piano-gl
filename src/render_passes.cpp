...
CameraState computeViewProj(const SceneGPU &scene, float aspect, float t,
    ...
    if (useKeyboardCamera) {
        ...
        cam.pos  = scene.sceneCenter + glm::vec3(std::cos(t * 0.35f) * radial, height,
                                                 std::sin(t * 0.35f) * radial);
        ...
        cam.pos  = scene.sceneCenter + glm::vec3(std::sin(t * 0.3f) * camDist,
                                                 scene.sceneRadius * 0.6f,
                                                 std::cos(t * 0.3f) * camDist);
        cam.view = glm::lookAt(cam.pos, scene.sceneCenter, glm::vec3(0.0f, 1.0f, 0.0f));
        cam.proj = glm::perspective(glm::radians(50.0f), aspect, 0.01f, camDist * 6.0f);
    }
    return cam;
...
LightViewData computeLightView(const SceneGPU &scene, const glm::vec3 &lightDir, int shadowMapSize) {
    ...
    if (farPlane <= nearPlane + 0.1f) farPlane = nearPlane + 50.0f;
...
static void renderSkybox(const SkyboxProgram &prog, GLuint skyboxVAO, GLuint envMap,
...
static glm::mat4 makeReflectionMatrix(const glm::vec3 &normal, const glm::vec3 &point) {
    ...
    glm::mat4 R(1.0f);
    R[0][0] =  1 - 2*n.x*n.x; R[0][1] = -2*n.x*n.y;    R[0][2] = -2*n.x*n.z;
    R[1][0] = -2*n.y*n.x;     R[1][1] =  1 - 2*n.y*n.y; R[1][2] = -2*n.y*n.z;
    R[2][0] = -2*n.z*n.x;     R[2][1] = -2*n.z*n.y;     R[2][2] =  1 - 2*n.z*n.z;
    R[3][0] = -2*d*n.x;       R[3][1] = -2*d*n.y;        R[3][2] = -2*d*n.z;
    return R;
}
...
void renderFrame(SceneGPU &scene, const GLProgram &pbrProgram,
    ...
    if (scene.keyRotationsDirty) uploadKeyRotations(scene);
    ...
    auto drawDC = [&](const DrawCommand &dc, const glm::mat4 &modelOverride,
                       bool writeStencil, bool disableCull) {
        auto instIt = scene.instancedMeshes.find(dc.meshIndex);
        const InstanceInfo *inst = (instIt != scene.instancedMeshes.end()) ? &instIt->second : nullptr;
    ...
    if (scene.backPlaneMeshIndex && scene.backPlaneModel) {
        glm::vec3 planePoint  = scene.backPlanePointWS.value_or(
            glm::vec3(*scene.backPlaneModel * glm::vec4(0, 0, 0, 1)));
        glm::vec3 planeNormal = scene.backPlaneNormalWS.value_or(
            glm::normalize(glm::vec3(*scene.backPlaneModel * glm::vec4(0, 0, 1, 0))));
